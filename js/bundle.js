...
function calc() {
    ...
    getStaticIncformation('.calculating__choose_big div','calculating__choose-item_active' );
...
function slider() {
    ...
    let slideIndex = 1;      //Индекс который будет определять текущее положение в слайдере
    let offset = 0;
    ...
    const slides = document.querySelectorAll('.offer__slide'),
     ...
     if (slides.length < 10){          //Подставляем 0 перед цифрой
     ...
     slides.forEach(slide =>{                                 // теперь все слайды одинаковой ширины
...
function tabs() {
    ...
    function hideTabContent(){     //Функция скрыть tabs
        tabsContent.forEach(item =>{
            item.classList.add('hide');
            item.classList.remove('show', 'fade');
        });

       tabs.forEach(item =>{      //Отключаем активность элемента
            item.classList.remove('tabheader__item_active');
       });
    }

    function showTabContent(i = 0) {    //Показываем tabs и добовляем активность
